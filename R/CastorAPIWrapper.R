...
CastorAPIWrapper <- R6::R6Class(
  ...
  public = list(
    ...
    access_token = NULL,
    type_to_func = list(
      date = function(date_val)
        as.Date(transform_missings_date(date_val), "%d-%m-%Y"),
      year = function(value)
        as.integer(transform_missings(value)),
      numeric = function(value)
        as.numeric(transform_missings(value)),
    ...
    initialize = function(username,
      ...
      if (missing(username) & missing(password) &
          !(missing(key) |
            missing(secret)) & is.null(access_token)) {
    ...
    getRequest = function(req_path,
                          query_data = list(),
                          raw = FALSE) {
      api_req_path <- paste0("api/", req_path)
      if (self$verbose)
        message("Getting ", file.path(self$base_url, req_path))
      ...
      if (!is.null(self$access_token)) {
        ...
        if (request$status_code == 401) {
          ...
          stop(
            "Check your credentials -- requests cannot be made with current credentials.",
            "\nError with request for ",
            request$url,
            "\nError code: ",
            request$status_code,
            "\nHeaders: ",
      ...
      if (raw) {
        ...
        result <- tryCatch({
          ...
          stop(
            "The request content could not be parsed to JSON.\n",
    ...
    collectPages = function(request_url,
                            page = NULL,
                            include = NULL,
                            page_size = NULL,
                            ...) {
      ...
      pages <-
        list(self$getRequest(request_url, query_data = query_params))
      ...
      if (!is.null(page)) {
        if (page > page_count | page < 1)
          stop(
            sprintf(
              "page out of range: requested page `%s` of `%s` pages available",
              page,
              page_count
            )
          )
      ...
      if (self$verbose)
        message("Total number of pages: ", page_count)
      if (self$verbose)
        message("Total number of items: ", pages[[1]]$total_items)

      # Get other pages and append them to the list.
      if (page_count > 1 & is.null(page)) {
        pages <- c(pages,
                   lapply(seq(2, page_count),
                          function(pagenum) {
    ...
    getForm = function(study_id, form_id) {
      # Returns a single study.
      study <-
        self$getRequest(paste0("study/", study_id, "/form/", form_id))
      return(study)
    },
    ...
    getVisit = function(study_id, visit_id) {
      # Returns a single study.
      study <-
        self$getRequest(paste0("study/", study_id, "/visit/", visit_id))
      return(study)
    },
    ...
    getSurvey = function(study_id, survey_id) {
      # Returns a single study.
      study <-
        self$getRequest(paste0("study/", study_id, "/survey/", survey_id))
      return(study)
    },
    getSurveysPages = function(study_id,
                               include = "forms",
                               page = NULL) {
    ...
    getRepeatingData = function(study_id, repeating_data_id, include = NULL) {
      ...
      self$getRequest(
        glue("study/{study_id}/repeating-data/{repeating_data_id}"),
    ...
    getRepeatingDataForm = function(study_id,
                                    repeating_data_id,
    ...
    getRepeatingDataFormsPages = function(study_id, repeating_data_id,
      ...
      repeating_data_forms <-
        self$collectPages(repeating_data_forms_url,
                          page = page)
    ...
    getFieldsPages = function(study_id,
                              include = "optiongroup",
                              page = NULL) {
    ...
    getParticipant = function(study_id, participant_id) {
      participant_url <-
        glue("study/{study_id}/participant/{participant_id}")
    ...
    getStudyDataPointsPages = function(study_id, participant_id, page = NULL) {
      sdp_url <-
        glue("study/{study_id}/participant/{participant_id}/study-data-point")
    ...
    getSurveyPackageInstanceMetadataPages = function(study_id, page = NULL) {
      spi_md_url <- glue("study/{study_id}/survey-package-instance")
      self$collectPages(spi_md_url, page = page)
    }
  ),

...
test_that("getVisit returns an appropriate object.", {
  visit <- castor_api$getVisit(creds$output_study, creds$example_visit)
...
test_that("getVisitsPages returns an appropriate object.", {
  visits <- castor_api$getVisitsPages(creds$output_study)
  one_visit <- castor_api$getVisitsPages(creds$output_study, page = 1)

  expect_is(visits, "list")
  expect_is(one_visit, "list")
  expect_true(length(one_visit) == 1)
  expect_error(castor_api$getVisitsPages(creds$output_study, page = -1))
  expect_error(castor_api$getVisitsPages(creds$output_study, page = 100000000))
  expect_error(castor_api$getVisitsPages(creds$output_study, page = pi))
})

test_that("getVisits returns an appropriate object.", {
  visit_data <- castor_api$getVisits(creds$output_study)
